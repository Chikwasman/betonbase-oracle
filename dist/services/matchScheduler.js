...
class MatchScheduler {
    ...
    async processUpcomingMatches() {
        logger_1.logger.info('🔄 Starting match scheduling process...');
        ...
        for (const leagueId of leagues) {
            ...
            try {
                ...
                const fixtures = await this.apiFootball.getUpcomingFixtures(leagueId, constants_1.CONFIG.MATCH_ADD_DAYS_BEFORE);
                totalFound += fixtures.length;
                if (fixtures.length === 0) {
                    logger_1.logger.info(`No upcoming matches found for ${leagueName}`);
                    continue;
                }
        ...
        logger_1.logger.info(`📊 Summary: Found=${totalFound}, Added=${totalAdded}, Skipped=${totalSkipped}, Errors=${totalErrors}`);
    }
    /**
     * Process a single fixture
     */
    async processFixture(fixture, leagueId, leagueName) {
        ...
        try {
            ...
            if (existingMatch && existingMatch.status === 'added') {
                logger_1.logger.debug(`Match ${apiMatchId} already added: ${homeTeam} vs ${awayTeam}`);
                return 'skipped';
            }
            // Check if match exists on blockchain
            const onChain = await this.blockchain.matchExists(apiMatchId);
            if (onChain) {
                logger_1.logger.info(`Match ${apiMatchId} already on blockchain: ${homeTeam} vs ${awayTeam}`);
                // Update database status
                this.database.updateMatch(apiMatchId, { status: 'added', addedAt: Date.now() });
                return 'skipped';
            }
            // Add match to blockchain
            logger_1.logger.info(`➕ Adding match: ${homeTeam} vs ${awayTeam} (${new Date(kickoffTime * 1000).toISOString()})`);
            ...
            if (success) {
                ...
                logger_1.logger.info(`✅ Successfully added match ${apiMatchId}`);
                return 'added';
            }
            else {
                logger_1.logger.error(`❌ Failed to add match ${apiMatchId}`);
            ...
            logger_1.logger.error(`Error processing fixture ${apiMatchId}: ${error.message}`);
            // Save error to database
            this.database.saveMatch({
                apiMatchId,
                homeTeam,
     ...
     */
    async checkMatchesForToday() {
        logger_1.logger.info('🔍 Checking for matches to add today...');
        const targetTimestamp = Date.now() + (constants_1.CONFIG.MATCH_ADD_DAYS_BEFORE * 24 * 60 * 60 * 1000);
        const targetDate = new Date(targetTimestamp);
        logger_1.logger.info(`Target date: ${targetDate.toISOString()}`);
...
exports.MatchScheduler = MatchScheduler;
